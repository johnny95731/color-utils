...
export const rgb2oklab = (rgb: readonly number[]) => {
  ...
  const l = Math.cbrt(
    8.189330101e-3 * x + 3.618667424e-3 * y + -1.288597137e-3 * z,
  );
  const m = Math.cbrt(
    0.329845436e-3 * x + 9.293118715e-3 * y + 0.361456387e-3 * z,
  );
  const s = Math.cbrt(
    0.482003018e-3 * x + 2.643662691e-3 * y + 6.338517070e-3 * z,
  );
  // To oklab
  return [
...
export const oklab2rgb = (oklab: readonly number[]) => {
  ...
  const b = oklab[2];
  // To lms (temporary state)
  let l_ = 0.9999999984505199 * l
    + 0.39633779217376786 * a
    + 0.2158037580607588 * b;
  let m_ = 1.0000000088817607 * l
    + -0.10556134232365634 * a
    + -0.0638541747717059 * b;
  let s_ = 1.000000054672411 * l
    + -0.08948418209496577 * a
    + -1.291485537864092 * b;
  l_ *= l_ * l_;
  m_ *= m_ * m_;
  s_ *= s_ * s_;
